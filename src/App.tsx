...
export default function App() {
  ...
  const [roomCode, setRoomCode] = useState(
    () => localStorage.getItem('clicker_roomCode') || ''
  );
  const [nickname, setNickname] = useState(
    () => localStorage.getItem('clicker_nickname') || ''
  );
  const [role, setRole] = useState(
    () => localStorage.getItem('clicker_role') || null
  );
        ...
        <div className="w-full max-w-2xl bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="space-y-3 pt-1">
                ...
                <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-xl">
                  <div>
                    <span className="block font-bold text-xs text-indigo-500 mb-1">
                      JAPANESE
                    </span>
                  ...
                  <div className="md:border-l border-slate-200 md:pl-4 border-t md:border-t-0 pt-3 md:pt-0">
                    ...
                    선생님과 학생은 동일한 「룸 코드(방 번호)」를 입력합니다.
                    예: CLASS101
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="space-y-3 pt-1">
                <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                  <Smartphone className="w-4 h-4 text-indigo-500" /> Student
                  (学生 / 학생)
                </h3>
                <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-xl">
                  <div>
                  ...
                  <div className="md:border-l border-slate-200 md:pl-4 border-t md:border-t-0 pt-3 md:pt-0">
                    ...
                    'Student'를 선택하고 닉네임을 입력해 참여합니다. 선생님이
                    문제를 내면 A~E 선택지 중 하나에 투표하세요.
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Step 3: Teacher */}
            <div className="flex gap-4 items-start">
              <div className="w-10 h-10 rounded-full bg-teal-50 flex items-center justify-center text-teal-600 font-bold shrink-0">
                3
              </div>
              <div className="space-y-3 pt-1">
                <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                  <School className="w-4 h-4 text-teal-500" /> Teacher (先生 /
                  선생님)
                </h3>
                <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-xl">
                  <div>
                  ...
                  <div className="md:border-l border-slate-200 md:pl-4 border-t md:border-t-0 pt-3 md:pt-0">
                    ...
                    'Teacher'를 선택해 방을 만듭니다. 실시간으로 집계 결과를
                    확인 및 공개하거나, 다음 문제로 진행할 수 있습니다.
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 md:p-12 rounded-3xl shadow-2xl w-full max-w-md border border-white/50">
          <div className="text-center mb-10 relative">
            <h1 className="text-4xl font-extrabold text-slate-800 tracking-tight mb-2">
              Clicker<span className="text-indigo-600">.io</span>
            </h1>
          ...
          <div className="space-y-6">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 rounded-xl bg-slate-50 border-2 border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none text-xl font-bold tracking-widest text-center uppercase transition-all"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              ...
              >
                ...
                <span className="font-bold text-slate-700 group-hover:text-indigo-700 relative z-10">
                  Student
                </span>
              </button>
              ...
              >
                ...
                <span className="font-bold text-slate-700 group-hover:text-teal-700 relative z-10">
                  Teacher
                </span>
              </button>
            </div>
          ...
          <div className="space-y-4">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 rounded-xl bg-slate-50 border-2 border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none text-lg font-medium transition-all"
                autoFocus
                onKeyDown={(e) =>
                  e.key === 'Enter' && nickname.trim() && handleJoinRoom()
                }
              />
            </div>
...
function Room({ user, roomCode, role, nickname, onLogout, db, appId }) {
  ...
  useEffect(() => {
    if (role === 'teacher' && !loading && !roomData) {
      const initRoom = async () => {
        ...
        try {
          await setDoc(roomRef, {
            ...
            responses: {},
            history: [],
            correctAnswer: null, // Added field
          });
        } catch (e) {
          console.error(e);
        }
  ...
  // Function for teacher to set correct answer
  const toggleCorrectAnswer = async (option) => {
    ...
    // If clicking same answer, unset it. Otherwise set it.
  ...
  const downloadCSV = () => {
    if (!roomData) return;
    let csvContent =
      'data:text/csv;charset=utf-8,Question,Nickname,Option,Time,UserID\n';
    const addRows = (responses, qNum) => {
      Object.entries(responses).forEach(([uid, data]) => {
        const safeName = data.nickname
          ? data.nickname.replace(/,/g, '')
          : 'No Name';
        csvContent += `${qNum},${safeName},${data.option},${data.timestamp},${uid}\n`;
      });
    };
    if (roomData.history && Array.isArray(roomData.history)) {
      roomData.history.forEach((entry) =>
        addRows(entry.responses, entry.question)
      );
    }
    if (roomData.responses) {
      addRows(roomData.responses, roomData.currentQuestion || 1);
    }
    ...
    link.setAttribute(
      'download',
      `results_${roomCode}_${new Date().toISOString().slice(0, 10)}.csv`
    );
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Current votes will be archived and the question number will
              advance to {currentQ + 1}.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-900 transition-colors"
              >
                Proceed
              </button>
            </div>
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-500 text-sm mb-6">
              This will delete all history and reset the question counter to 1.{' '}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={resetRoom}
                className="flex-1 py-2 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition-colors shadow-lg shadow-red-500/30"
              >
                Reset All
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-xs w-full animate-in zoom-in-95 duration-200">
            ...
            <input
              ...
              className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-center text-2xl outline-none focus:border-teal-500 mb-4"
              min="1"
              autoFocus
            />
            <button
      ...
      >
        <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-4 overflow-hidden">
            <div
              className={`w-10 h-10 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center text-white shadow-lg shadow-${themeColor}-500/20 flex-shrink-0`}
            >
              {isTeacher ? (
            ...
            <div className="min-w-0">
              <h1 className="font-bold text-lg leading-none flex items-center gap-2">
                <span className="truncate">
                  {isTeacher ? 'Admin Dashboard' : 'Voting Terminal'}
                </span>
                ...
                {isTeacher ? (
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-md text-xs font-mono bg-${themeColor}-50 text-${themeColor}-700 border border-${themeColor}-100`}
                  >
                    Q{currentQ}
                  </span>
                )}
              ...
              <p className="text-xs text-slate-400 font-medium mt-1 truncate font-mono">
                {roomCode} <span className="text-slate-300">|</span>{' '}
                {nickname || 'Admin'}
              </p>
            </div>
          </div>
      ...
      <main className="flex-grow p-4 md:p-8 w-full max-w-5xl mx-auto">
        ...
        {isTeacher && (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
            ...
            <div className="md:col-span-8 bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col justify-between">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  {isRevealed ? (
                    <>
                      <EyeOff className="w-5 h-5" /> Hide Results
                    </>
                  ) : (
                    <>
            ...
            <div className="md:col-span-4 bg-slate-50 p-6 rounded-3xl border border-slate-200 flex flex-col gap-3 justify-center">
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider text-center mb-2">
                Session Tools
              </p>
              <button
                onClick={downloadCSV}
        ...
        {!isTeacher && roomData.status === 'voting' && (
          <div className="max-w-md mx-auto py-4">
            <div className="text-center mb-8">
              <span className="inline-block px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-xs font-bold mb-2 border border-indigo-100">
                QUESTION {currentQ}
              </span>
          ...
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-2xl shadow-slate-200/50 border border-slate-100 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="h-64 md:h-80 flex items-end justify-between gap-2 md:gap-6 px-2 pb-2">
              {['A', 'B', 'C', 'D', 'E'].map((opt, index) => {
                ...
                const barGradient = isCorrect
                  ? 'from-green-400 to-emerald-500 shadow-emerald-500/50'
                  : gradients[index];
                  ...
                  >
                    ...
                    <div className="relative flex-1 w-full flex items-end justify-center bg-slate-50 rounded-2xl overflow-hidden">
                      ...
                      <div
                        ...
                        style={{
                          height: `${percentage === 0 ? 2 : percentage}%`,
                        }}
                      >
            ...
            {!isTeacher && isRevealed && correctAnswer && (
              <div
                className={`mt-6 p-4 rounded-xl text-center border-2 animate-in zoom-in duration-300 ${
